...
static int rkcif_shm_vmap(struct dma_buf *dma_buf, struct dma_buf_map *map)
{
	struct rkcif_shm_data *data = dma_buf->priv;

	data->vaddr = vmap(data->pages, data->npages, VM_MAP, PAGE_KERNEL);
	data->vmap_cnt++;
	dma_buf_map_set_vaddr(map, data->vaddr);
	return 0;
}

static void rkcif_shm_vunmap(struct dma_buf *dma_buf, struct dma_buf_map *map)
{
	struct rkcif_shm_data *data = dma_buf->priv;
